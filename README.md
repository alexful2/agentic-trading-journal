...
| `execution-audit` | Adversarial audit of an execution-thesis (run on a different model) - check the validity of hypotheses formed and audit back into execution-thesis file (after running company projects, execution thesis, and execution thesis audit, the file "execution-thesis" is the final one to look at). | `vault/companies/TICKER/execution-audit.md` |
